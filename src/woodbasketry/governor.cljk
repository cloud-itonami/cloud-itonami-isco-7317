(ns woodbasketry.governor
  "WoodBasketryGovernor — the independent safety/scope layer gating
  every workshop scheduling/logistics proposal an advisor may make for
  a wood-and-basketry handicraft workshop crew. The governor never
  dispatches hardware itself, never performs the craft fabrication
  work on the workshop floor, and never finalizes a
  craft-fabrication-execution decision (e.g. deciding to proceed with
  a specific carving, weaving or finishing step) or overrides a
  workshop safety officer's judgment — those are permanently out of
  this actor's scope and remain a workshop safety officer's exclusive
  judgment (README's 'Robotics premise': this actor coordinates
  WORKSHOP SCHEDULING/LOGISTICS ONLY — it never performs the craft
  fabrication work itself). Modeled on cloud-itonami-isco-7111's
  housebuilder.governor (and closely on cloud-itonami-isco-9311's
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs the craft fabrication
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                craft-fabrication-execution decision
                                (e.g. deciding to proceed with a
                                specific carving, weaving or finishing
                                step), or to override a workshop
                                ...
                                \"proceed with the craft-fabrication
                                work\" / \"finalize the weaving
                                decision\" / \"override the workshop
                                safety officer's judgment\", never as
                                bare nouns like \"craft\", \"wood\",
                                \"basketry\", \"carving\", \"weaving\"
                                ...
                                crew operation for basketry-weaving
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a cut-hazard / dust-exposure /
                                repetitive-strain-injury concern always
                                escalates to a human, never
                                auto-commits).
    7. :op :coordinate-supply-order above `supply-cost-threshold`.
    8. low confidence (< `confidence-floor`)."
  (:require [kotoba.lang.text :as str]
            [woodbasketry.store :as store]))
...
(def ^:private scope-excluded-ops
  #{:finalize-craft-fabrication-decision :finalize-carving-decision
    :proceed-with-craft-fabrication :override-safety-officer-judgment
    :override-workshop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("craft", "wood", "basketry", "carving", "weaving", "safety") — so
...
;; "basketry-weaving task" / "workshop safety officer review"). See
;; `governor-test/default-mock-advisor-proposals-never-self-trip-on-scope-exclusion`.
(def ^:private scope-excluded-phrases
  ["proceed with the craft-fabrication work" "proceed with the carving work"
   "proceed with the weaving work"
   "finalize the craft-fabrication decision" "finalize the carving decision"
   "finalize the weaving decision"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "工芸品制作の実行判断（彫刻/編組等の工程）の確定・workshop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `woodbasketry.store/Store`. Pure — never
