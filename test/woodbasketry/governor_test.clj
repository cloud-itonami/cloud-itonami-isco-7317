(ns woodbasketry.governor-test
  (:require [clojure.test :refer [deftest is testing]]
            [woodbasketry.store :as store]
            [woodbasketry.advisor :as advisor]
            [woodbasketry.governor :as governor]))

(defn- fresh-store []
  (let [st (store/mem-store)]
    (store/register-worker! st {:worker-id "worker-1" :name "Kobo Yamada"})
    (store/register-workshop! st {:workshop-id "WS-1" :name "Kobo Basketry Workshop" :max-supply-cost 2000})
    st))
...
(deftest hard-on-scope-excluded-op-finalize-craft-fabrication-decision
  (testing "finalizing a craft-fabrication-execution decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-craft-fabrication-decision) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))

(deftest hard-on-scope-excluded-op-finalize-carving-decision
  (testing "finalizing a carving decision is a permanent block, never a routine op"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :finalize-carving-decision) :confidence 0.99) st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest hard-on-scope-excluded-rationale-even-with-allowlisted-op
  (testing "defense-in-depth: a rationale that itself attempts to proceed with the craft-fabrication work is blocked even if the op looks routine"
    (let [st (fresh-store)
          v (governor/check req {} (assoc (op :schedule-crew-operation)
                                           :rationale "recommend we proceed with the craft-fabrication work now")
                             st)]
      (is (:hard? v))
      (is (some #(= :scope-excluded-action (:rule %)) (:violations v))))))
...
(deftest always-escalates-safety-concern-even-at-high-confidence
  (testing "a cut-hazard/dust-exposure/repetitive-strain-injury concern always requires human sign-off"
    (let [st (fresh-store)
...
(deftest default-mock-advisor-proposals-never-self-trip-on-scope-exclusion
  (testing "the governor's scope-exclusion term list must never match the mock advisor's own default rationale text for any allowlisted op — CLAUDE.md's known self-tripping bug pattern (rationale legitimately contains bare nouns like 'craft'/'wood'/'basketry'/'carving'/'weaving'/'safety officer', but never the full finalization-action phrases)"
    (let [st (fresh-store)
      ...
      (doseq [o ops]
        (let [request {:worker-id "worker-1" :op o :workshop-id "WS-1"
                        :stake :low :task "routine weaving task"
